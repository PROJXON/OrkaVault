  ...
  return (
    <div className="flex items-center space-x-2">
      ...
      <button
        ...
        } relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-hidden disabled:opacity-50`}
      >
        <span
          className={`${
            user.notificationsOn ? "translate-x-4" : "translate-x-0"
          } pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white dark:bg-[var(--bg-surface)] shadow-sm ring-0 transition duration-200 ease-in-out`}
        />
      </button>
    </div>
  );
