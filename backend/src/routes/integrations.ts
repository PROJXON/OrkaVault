...
router.post("/gchat/events", async (req: Request, res: Response) => {
  ...
  const email: string | undefined = event?.user?.email;
  const action = event?.action;

  if (event?.type !== "CARD_CLICKED" || !action || !email) {
    res.json({ text: "OrkaVault: nothing to do with this event." });
    return;
  }

  const fn: string = action.actionMethodName;
  const requestId: string | undefined = action.parameters?.find((p: any) => p.key === "requestId")?.value;
  if ((fn !== "approve" && fn !== "deny") || !requestId) {
    res.json({ text: "Unrecognized action." });
    return;
  }

  const user = await prisma.user.findUnique({ where: { email }, include: { managedCollections: true } });
  if (!user) {
    res.json({ text: "No OrkaVault account matches your Workspace email." });
    return;
  }
  if (user.role !== "MANAGER" && user.role !== "ADMIN") {
    res.json({ text: "Your OrkaVault account doesn't have permission to approve or deny requests." });
    return;
  }
  const actor: RequestActor = { id: user.id, role: user.role, managedCollections: user.managedCollections };

  try {
    const result =
      fn === "approve"
        ? await approveAccessRequest(actor, requestId, req.ip, "gchat")
        : await denyAccessRequest(actor, requestId, "Denied via Google Chat", req.ip, "gchat");
    const verb = fn === "approve" ? "Approved" : "Denied";
    res.json({ text: `✅ ${verb} — ${result.accountName} / ${result.requesterName} (by ${user.name})` });
  } catch (error: any) {
    const message = error instanceof RequestActionError ? error.message : "Failed to process this action.";
    res.json({ text: `⚠️ ${message}` });
  }
});
